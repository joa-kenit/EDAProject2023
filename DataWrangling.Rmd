...
Plot2.get <- function(parameter1, parameter2) {
  plot <- water.durham.wrangle %>%
    filter(Parameter == "Iron" | Parameter == "Manganese" ) %>%
    ggplot(
      mapping = aes(
        x=Date,
        y=Value,
  ...
  labs(title = paste0("Relationship between - ", "Iron", " and ", "Manganese"), x = "Year", y=paste0("Value"," ","(", pull(water.durham.wrangle[water.durham.wrangle$Parameter == "Iron", "Unit"])[2],")")) +
    my_theme+
    #Sys.sleep(1) # Scraping etiquette
    return(plot)
}
